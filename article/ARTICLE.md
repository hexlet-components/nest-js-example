...
Метод getHello() и конструктор мы пока удалим.

## Статьи

Итак, создадим страницу со списком статей. Но что такое - статья?

Создадим в папке *src* новый файл *article.model.ts* с таким содержимым:
...
Для того, чтобы пользователю было комфортнее работать с нашими постами, сервер должен вернуть браузеру ответ не в формате json, а в виде html-страницы. Для этого нам понадобятся шаблоны. Создадим в корне проекта папку *views*.

Nest.js так же, как *express.js* позволяет нам использовать любой шаблонизатор. Мы для этого примера возьмём pug (ранее известный, как jade).
...
Перепишем функцию bootstrap (*main.ts*) следующим образом:
